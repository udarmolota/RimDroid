...
int rimdroid_run_elf(const char* path, int argc, const char** argv) {
    ...
    // Our argv already has path at [0]; prepend a fake "box64" slot.
    int init_argc = argc + 1;
    const char** init_argv = malloc((size_t)init_argc * sizeof(char*));
    if (!init_argv) return -1;
    init_argv[0] = get_self_path();
    for (int i = 0; i < argc; i++) init_argv[i + 1] = argv[i];

    x64emu_t*    emu        = NULL;
    elfheader_t* elf_header = NULL;

    LOGI("[emulation] calling box64 initialize() for %s", path);
    if (initialize(init_argc, init_argv, environ, &emu, &elf_header, 1) != 0) {
